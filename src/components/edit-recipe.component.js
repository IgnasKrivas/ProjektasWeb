...
export default class Edit extends Component { 
      ...
      .catch(function (error) {
        ...
        .then(response => {
            if( response.data.length > 0){
                this.setState({
                    ingredient: response.data.map(ing => [ing._id, ing.title, ing.calories])
                })
            }
        })
        .catch((error) => {
    ...
    onSubmit(e) {
        ...
        console.log(recipe);
        axios.patch('http://localhost:8080/recipes/' + this.props.match.params.id, recipe)
    ...
    render() {
        return (
          ...
          <select multiple ref="userInput"
              ...
              <div className="form-group">
                <input type="submit" value="Atnaujinti receptą" className="btn btn-success" />
