...
router.delete('/:ingredientId', async (req, res) => {
    try{
        const removeIngredient = await Ingredient.remove({ _id: req.params.ingredientId });
...
router.patch('/:ingredientId', async (req, res) => {
    try{
        const updateIngredient = await Ingredient.updateOne({ _id: req.params.ingredientId }, { $set: {title: req.body.title}});
